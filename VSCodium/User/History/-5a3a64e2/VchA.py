...
class Gui:
    ...
    def __init__(self, pos_x: int=0, pos_y: int=0):
        ...
        self.duration_scrollbar = ttk.Scale(master = self.window, orient = 'horizontal', from_ = decimal.Decimal('0.1'), to = decimal.Decimal('60.0'), value = Settings.duration, command = self.update_duration)
        self.keybind_button = ttk.Button(master = self.window, text = 'Change keybind', command = self.change_keybind) 
