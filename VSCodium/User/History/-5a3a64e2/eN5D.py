...
class Gui:
    ...
    def update_fov():
        Settings.fov = decimal.Decimal(Gui.fov_spinbox.get())
        Gui.fov_label['text'] = f'FOV: {Settings.fov}'

    def update_sensitivity():
        Settings.in_game_sensitivity = decimal.Decimal(Gui.sensitivity_spinbox.get())
    ...
    def __init__(self, pos_x: int=0, pos_y: int=0):
        self.window = tk.Tk()
        self.script_label = ttk.Label(master = window, text = 'Rotation Script', font = 'Ubuntu 18')

        self.fov_label = ttk.Label(master = window, text = f'FOV: {Settings.fov}')
        self.sensitivity_label = ttk.Label(master = window, text = f'Sensitivtiy: {Settings.in_game_sensitivity}')
        self.duration_label = ttk.Label(master = window, text = f'Duration time: {Settings.duration}')
        self.keybind_label = ttk.Label(master = window, text = f'Keybind: {Settings.keybind}')
        self.window = tk.Tk()
        self.script_label = ttk.Label(master = window, text = 'Rotation Script', font = 'Ubuntu 18')
        ...
        self.fov_spinbox = ttk.Spinbox(master = window, from_ = decimal.Decimal('10.0'), to = decimal.Decimal('180.0'), increment = 0.1, command = update_fov, validatecommand = (window.register(float_validate)))
        self.sensitivity_spinbox = ttk.Spinbox(master = window, from_ = decimal.Decimal('0.01'), to = decimal.Decimal('10.0'), increment = 0.01, command = update_sensitivity, validate = 'all' ,validatecommand = (window.register(float_validate)))
        self.duration_scrollbar = ttk.Scale(master = window, orient = 'horizontal', from_ = decimal.Decimal('0.1'), to = decimal.Decimal('60.0'), value = Settings.duration, command = update_duration)
        self.keybind_button = ttk.Button(master = window, text = 'Change keybind', command = change_keybind) 
