...
class Gui:
    ...
    def __init__(self, pos_x: int=0, pos_y: int=0):
        ...
        self.fov_spinbox = ttk.Spinbox(master = self.window, from_ = decimal.Decimal('10.0'), to = decimal.Decimal('180.0'), increment = 0.1, command = update_fov, validatecommand = (window.register(float_validate)))
        self.sensitivity_spinbox = ttk.Spinbox(master = self.window, from_ = decimal.Decimal('0.01'), to = decimal.Decimal('10.0'), increment = 0.01, command = update_sensitivity, validate = 'all' ,validatecommand = (window.register(float_validate)))
        self.duration_scrollbar = ttk.Scale(master = self.window, orient = 'horizontal', from_ = decimal.Decimal('0.1'), to = decimal.Decimal('60.0'), value = Settings.duration, command = update_duration)
        self.keybind_button = ttk.Button(master = self.window, text = 'Change keybind', command = change_keybind) 
